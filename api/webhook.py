        ...
        if "message" in body:
            ...
            if text:
                try:
                    ...
                    print(f"handle_msg error: {traceback.format_exc()}")
                    send(uid, f"❌ Erreur: `{str(e)[:150]}`")
            ...
            try:
                ...
                print(f"handle_cb error: {traceback.format_exc()}")
    except Exception as e:
...
def handle_msg(uid, uname, text):
    ...
    if text.startswith("/start"):
        ...
        send(uid,
            ...
            f"  🎁 Bonus journalier\n"
            f"  👥 Parrainage\n"
            f"  ✅ Taches quotidiennes\n\n"
            f"📌 Rejoins nos canaux :\n{channels_list}\n\n"
            f"📌 Puis clique ✅",
            kb={"inline_keyboard":[[{"text":"✅ J'ai tout rejoint — Verifier","callback_data":"check_join"}]]}
        )
        return
    ...
    if uid in ADMIN_IDS:
        if text in ["/admin", "📊 Statistiques"]:
            ...
            send(uid,
                ...
                f"✅ Paye : *{sum(x.get('amount',0) for x in ws if x.get('status')=='approved')}F*\n"
        ...
        if text == "⚙️ Prix":
            send(uid,
                f"⚙️ *CONFIG*\n\n"
                f"🎁 Bonus : *{get_cfg('bonus_daily')}F*\n"
                f"👥 Parrainage : *{get_cfg('bonus_referral')}F*\n"
                f"✅ Tache : *{get_cfg('bonus_task')}F*\n"
        ...
        if text == "➕ Ajouter Tache":
            set_session(uid,{"action":"add_task","step":"description"}); send(uid,"➕ *NOUVELLE TACHE*\n\nDecris la tache :"); return
        if text == "💸 Retraits":
            ...
            return
        if text == "🚫 Bannir":
            set_session(uid,{"action":"ban"}); send(uid,"🚫 ID a bannir :\n`123456789`\nDebannir : `debannir 123456789`"); return
        if text == "📢 Broadcast":
            set_session(uid,{"action":"broadcast"}); send(uid,"📢 Ecris le message :"); return
        if text == "🔙 Mode User":
    ...
    if text == "🎁 Bonus":
        ...
        send(uid,
            ...
            f"💵 Total : *{nb*bonus_ref}F*\n\n"
            f"📤 _Partage et gagne {bonus_ref}F a chaque inscription !_ 🚀")
        ...
        for t in tasks:
            ...
            if t.get("link"): msg_text += f"   🔗 {t['link']}\n"
            msg_text += f"   💰 *{t['reward']}F*\n\n"
            if not done: buttons.append([{"text":f"✅ {t['description'][:30]}","callback_data":f"task_{t['id']}"}])
        ...
        if not ranked: t += "_Sois le premier !_ 🚀"
        send(uid, t)
...
def handle_retrait_step(uid, text, sess):
    ...
    if step == "amount":
        try: amount = int(text.strip())
            ...
            send(uid,"❌ Montant invalide (ex: 2500)"); return
        if amount < min_w:
        ...
        if len(text.strip()) < 8:
            send(uid,"❌ Numero invalide."); return
        sess["number"] = text.strip(); sess["step"] = "name"
        set_session(uid, sess)
        ...
        amount = sess["amount"]; method = sess["method"]
        number = sess["number"]; name = sess["name"]
    ...
    if data == "check_join":
        ...
        if u.get("referred_by") and not u.get("is_registered"):
            ...
            if parrain and parrain.get("is_registered") and not parrain.get("is_banned"):
                ...
                try: send(u["referred_by"],f"🎉 *+{bonus_ref}F* — Filleul *@{u.get('username','?')}* inscrit !")
                except: pass
        ref_link = f"https://t.me/{BOT_USERNAME}?start={uid}"
        ...
        if db_get("user_tasks",{"user_id":f"eq.{uid}","task_id":f"eq.{task_id}"}):
            tg("answerCallbackQuery",callback_query_id=str(mid),text="⚠️ Deja completee !",show_alert=True); return
        ...
        if not ws or ws[0].get("status") != "pending": return
        w = ws[0]
        ...
        if decision == "approve":
            ...
            try: send(w["user_id"],f"✅ *Retrait approuve !*\n\n💵 *{w['amount']}F* envoye ! 🙏")
            except: pass
        else:
            ...
            try: send(w["user_id"],f"❌ *Retrait refuse*\n\n💵 *{w['amount']}F* rembourse.\n📩 @xafearn_support")
            except: pass
