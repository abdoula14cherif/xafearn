...
PAYS_METHODES = {
    "\U0001f1f9\U0001f1ec Togo":         ["Flooz", "T-Money"],
    "\U0001f1e8\U0001f1ee Cote d Ivoire":["Orange Money", "Wave", "MTN Money", "Moov Money"],
    "\U0001f1f8\U0001f1f3 Senegal":      ["Orange Money", "Wave", "Free Money"],
    "\U0001f1e7\U0001f1eb Burkina Faso": ["Orange Money", "Moov Money"],
    "\U0001f1f2\U0001f1f1 Mali":         ["Orange Money", "Moov Money"],
    "\U0001f1e7\U0001f1ef Benin":        ["MTN Money", "Moov Money"],
    "\U0001f1e8\U0001f1f2 Cameroun":     ["Orange Money", "MTN Money"],
    "\U0001f1ec\U0001f1f3 Guinee":       ["Orange Money", "MTN Money"],
    "\U0001f1f3\U0001f1ea Niger":        ["Airtel Money", "Moov Money"],
    "\U0001f1e8\U0001f1ec Congo":        ["Airtel Money", "MTN Money"],
    "\U0001f1ec\U0001f1e6 Gabon":        ["Airtel Money", "Moov Money"],
    "\U0001f1f9\U0001f1e9 Tchad":        ["Airtel Money", "Moov Money"],
    "Autre":                             ["Virement Bancaire", "PayPal"],
    "\U0001f4b0 Crypto":                 ["USDT TRC20", "USDT BEP20 (BSC)", "BNB BSC", "SOL (Solana)"]
}
...
def main_kb():
    return {"keyboard": [
        ["\U0001f381 Bonus", "\U0001f465 Parrainage"],
        ["\U0001f4b0 Solde", "\U0001f4cb Historique"],
        ["\U0001f4b8 Retrait", "\U00002705 Taches"],
        ["\U0001f3c6 Classement", "\U00002753 Aide"],
        [{"text": "\U0001f680 Ouvrir XAFEARN App", "web_app": {"url": MINIAPP_URL}}]
    ], "resize_keyboard": True}

def admin_kb():
    return {"keyboard": [
        ["\U0001f465 Tous les Users", "\U0001f4ca Statistiques"],
        ["\U00002699\U0000fe0f Prix", "\U00002795 Ajouter Tache"],
        ["\U0001f4b8 Retraits", "\U0001f6ab Bannir"],
        ["\U0001f4b5 Ajouter Solde", "\U0001f4e2 Broadcast"],
        ["\U0001f5d1\U0000fe0f Supprimer Tache", "\U000026d4 Pause Retraits"],
        ["\U0001f50d Verif Canaux", "\U0001f4e2 Rappel Bonus"],
        ["\U0001f4cb Log Actions", "\U0001f519 Mode User"]
    ], "resize_keyboard": True}
...
def methodes_keyboard(pays):
    ...
    buttons = [[{"text": m, "callback_data": "methode_" + str(i) + "_" + pays}] for i, m in enumerate(methodes)]
    buttons.append([{"text": "Annuler", "callback_data": "cancel_retrait"}])
    return {"inline_keyboard": buttons}
...
def db_del(table, f):
    try: req.delete(f"{DB}/{table}", headers=H, params=f, timeout=15)
    except: pass
...
def get_cfg(key):
    ...
    return {"bonus_daily":50,"bonus_referral":75,"bonus_task":35,"min_withdrawal":1000}.get(key, 0)
...
def set_session(uid, data):
    try:
        r = db_get("sessions", {"user_id": f"eq.{uid}"})
        if r: db_patch("sessions", {"user_id": f"eq.{uid}"}, {"data": json.dumps(data)})
        else: db_post("sessions", {"user_id": uid, "data": json.dumps(data)})
    except: pass

def clear_session(uid):
    try: db_del("sessions", {"user_id": f"eq.{uid}"})
    except: pass

def mask_number(number):
    n = number.strip()
    return n[:4] + " *** ** ** " + n[-2:] if len(n) >= 6 else n
...
def log_action(admin_id, action, detail=""):
    try:
        db_post("admin_logs", {
            ...
            "detail": str(detail)[:200], "created_at": datetime.now().isoformat()
        })
    except: pass

def ad_seen_today(uid):
    u = get_user(uid)
    return str(u.get("last_ad_view","")) == str(date.today()) if u else False

def mark_ad_seen(uid):
    db_patch("users", {"user_id": f"eq.{uid}"}, {"last_ad_view": str(date.today())})

def show_ad_then(uid, cb_data, preview):
    req.post(f"{API}/sendMessage", json={
        "chat_id": uid,
        "text": "Message sponsorise\n\nClique Voir l annonce\npuis Continuer\n\n" + preview,
        "reply_markup": {"inline_keyboard": [
            [{"text": "Voir l annonce", "url": AD_URL}],
            [{"text": "Continuer", "callback_data": cb_data}]
        ]}
    }, timeout=10)
...
def check_retrait_security(uid, amount, number):
    ...
    if pause and pause[0].get("value") == "1":
        return False, "Les retraits sont temporairement suspendus. Merci de ta patience."
    if number:
        clean = number.strip().replace(" ","")
        for blocked in BLACKLIST_NUMEROS:
            if clean == blocked.replace(" ",""):
                return False, "Une verification est en cours. Contacte le support."
    last_ws = db_get("withdrawals", {"user_id": f"eq.{uid}", "status": "eq.approved", "order": "requested_at.desc", "limit": "1"})
    if last_ws:
        ...
        if last_str:
            try:
                ...
                if days_since < DELAI_ENTRE_RETRAITS:
                    ...
                    return False, "Prochain retrait dans " + str(remaining) + " jour(s)."
            except: pass
    ...
    if week_count >= MAX_RETRAIT_PAR_SEMAINE:
        return False, "Limite de retraits atteinte pour cette semaine."
    if amount > 0:
        ...
        if today_total + amount > MAX_RETRAIT_PAR_JOUR:
            ...
            if remaining <= 0: return False, "Limite journaliere atteinte. Reviens demain."
            return False, "Tu peux retirer au maximum " + str(remaining) + "F aujourd hui."
    return True, None
        ...
        for w in all_ws:
            if w.get("user_id") != uid:
                for admin_id in ADMIN_IDS:
                    try: send(admin_id, "ALERTE MULTI-COMPTES\n\nNumero " + mask_number(number) + "\nUser actuel : " + str(uid) + "\nAutre user : " + str(w.get("user_id","?")))
                    except: pass
                return True
    except: pass
    return False
    ...
    for u in users:
        if str(u.get("last_bonus","")) != today:
            try:
                ...
                tg("sendMessage", chat_id=u["user_id"],
                   text="Rappel XAFEARN\n\nTu n as pas encore recupere ton bonus !\n\nClique Bonus pour recevoir +" + str(bonus) + "F")
                sent += 1
            except: pass
    return sent
...
def notif_palier(uid, balance):
    ...
    if balance >= mi and (balance - 50) < mi:
        try: send(uid, "Tu es a mi-chemin !\n\nSolde : " + str(balance) + "F\nPlus que " + str(min_w - balance) + "F pour retirer !")
        except: pass

def notif_bienvenue(uid, uname):
    bonus     = get_cfg("bonus_daily")
    ...
    try:
        send(uid,
            ...
            "2. Partage ton lien :\n" + ref_link + "\nGagne " + str(bonus_ref) + "F par ami inscrit !\n\n"
...
def handle_msg(uid, uname, text):
    ...
    if sess.get("action") == "confirm_code":
        if text.strip() == sess.get("code"):
            clear_session(uid)
            set_session(uid, {"action":"retrait","step":"amount","pays":sess.get("pays",""),"methode":sess.get("methode","")})
            u = get_user(uid); min_w = get_cfg("min_withdrawal")
            send(uid, "Code confirme !\n\nSolde : "+str(u["balance"])+"F\nMinimum : "+str(min_w)+"F\n\nCombien veux-tu retirer ? (en F)")
        else:
    ...
    if text.startswith("/start"):
        ...
        if not u:
            db_post("users", {"user_id":uid,"username":uname,"referred_by":referred_by,"balance":0,"is_banned":False,"is_registered":False})
        msg  = "Bienvenue sur XAFEARN " + str(uname) + "!\n\n"
        msg += "Gagne de l argent chaque jour :\n- Bonus journalier\n- Parrainage\n- Taches quotidiennes\n\n"
        msg += "Rejoins nos 3 canaux puis clique Verifier"
        req.post(f"{API}/sendMessage", json={
    ...
    u = get_user(uid)
    if not u: send(uid, "Utilise /start pour t inscrire."); return
    if u.get("is_banned"): send(uid, "Compte suspendu."); return

    today = str(date.today())

    # ══ ADMIN ══════════════════════════════════════════════
    if uid in ADMIN_IDS:
        if text in ["/admin", "\U0001f4ca Statistiques"]:
            users = db_get("users"); ws = db_get("withdrawals")
            pause = db_get("config", {"key": "eq.retraits_pauses"})
        ...
        if text == "\U000026d4 Pause Retraits":
            pause = db_get("config", {"key": "eq.retraits_pauses"})
            curr = pause[0].get("value","0") if pause else "0"
            if curr == "1": set_cfg("retraits_pauses","0"); send(uid,"Retraits REACTIVES."); log_action(uid,"RETRAITS_REACTIVES")
            else: set_cfg("retraits_pauses","1"); send(uid,"Retraits PAUSES."); log_action(uid,"RETRAITS_PAUSES")
            return

        if text == "\U0001f4cb Log Actions":
            logs = db_get("admin_logs", {"order":"created_at.desc","limit":"15"})
            if not logs: send(uid,"Aucun log disponible.")
            else:
                t = "HISTORIQUE ACTIONS ADMIN\n\n"
                for l in logs:
                    d = str(l.get("created_at",""))[:16]
                    t += d + " - " + str(l.get("action","")) + "\n"
                    if l.get("detail"): t += "  " + str(l.get("detail",""))[:50] + "\n"
                send(uid, t)
            return

        if text == "\U0001f50d Verif Canaux":
            users = db_get("users", {"is_registered":"eq.true","is_banned":"eq.false"})
            ...
            if not not_in: send(uid, "Tous les users actifs sont dans les canaux.")
            else:
                t = "USERS HORS CANAL (" + str(len(not_in)) + ")\n\n"
                for uu in not_in: t += "@" + str(uu.get("username","N/A")) + " - " + str(uu.get("balance",0)) + "F\n"
                send(uid, t)
                log_action(uid, "VERIF_CANAUX", "desactives="+str(len(not_in)))
            return

        if text == "\U0001f4e2 Rappel Bonus":
            sent = notif_rappel_bonus()
            send(uid, "Rappel envoye a "+str(sent)+" utilisateurs.")
            log_action(uid,"RAPPEL_BONUS","sent="+str(sent)); return

        if text == "\U0001f465 Tous les Users":
            users = db_get("users")
            t = "UTILISATEURS ("+str(len(users))+")\n\n"
            for uu in users[:20]:
                s = "BANNI" if uu.get("is_banned") else ("OK" if uu.get("is_registered") else "ATTENTE")
                t += s + " @" + str(uu.get("username","N/A")) + " - " + str(uu.get("balance",0)) + "F\n"
            send(uid, t); return

        if text == "\U00002699\U0000fe0f Prix":
            send(uid,
        ...
        if text.startswith("/setbonus "): set_cfg("bonus_daily",text.split()[1]); log_action(uid,"SET_BONUS",text.split()[1]); send(uid,"Bonus -> "+text.split()[1]+"F"); return
        if text.startswith("/setref "): set_cfg("bonus_referral",text.split()[1]); log_action(uid,"SET_REF",text.split()[1]); send(uid,"Parrainage -> "+text.split()[1]+"F"); return
        if text.startswith("/settask "): set_cfg("bonus_task",text.split()[1]); log_action(uid,"SET_TASK",text.split()[1]); send(uid,"Tache -> "+text.split()[1]+"F"); return
        if text.startswith("/setmin "): set_cfg("min_withdrawal",text.split()[1]); log_action(uid,"SET_MIN",text.split()[1]); send(uid,"Min retrait -> "+text.split()[1]+"F"); return

        if text == "\U00002795 Ajouter Tache":
            set_session(uid, {"action":"add_task","step":"description"})
            send(uid, "NOUVELLE TACHE\n\nDecris la tache :"); return

        if text == "\U0001f5d1\U0000fe0f Supprimer Tache":
            tasks = db_get("tasks", {"date":f"eq.{today}","is_active":"eq.true"})
            if not tasks: send(uid, "Aucune tache a supprimer.")
            else:
                t = "Taches du jour :\n\n"; buttons = []
                for task in tasks:
        ...
        if text == "\U0001f4e2 Broadcast":
            ...
            send(uid, "Ecris le message a envoyer a tous :\n\nLe bouton Publier mon annonce sera ajoute."); return

        if text == "\U0001f519 Mode User":
    ...
    if text == "\U0001f381 Bonus":
        ...
        if not ad_seen_today(uid):
            mark_ad_seen(uid)
            show_ad_then(uid, "ad_solde", "Ton solde actuel : "+str(u["balance"])+"F")
            return
        send(uid,
            "TON COMPTE XAFEARN\n\nSolde : "+str(u["balance"])+"F\n"
    ...
    elif text == "\U0001f4cb Historique":
        ...
        for w in (wds or []): t += se.get(w.get("status"),"?")+" "+str(w["amount"])+"F\n"
        if not wds: t += "Aucun.\n"
        send(uid, t)

    elif text == "\U0001f4b8 Retrait":
        min_w = get_cfg("min_withdrawal")
        if u["balance"] < min_w:
        ...
        if pending: send(uid, "Tu as deja une demande en attente de "+str(pending[0]["amount"])+"F"); return
        ok, msg = check_retrait_security(uid, 0, "")
        if not ok: send(uid, msg); return
        send(uid, "DEMANDE DE RETRAIT\n\nSolde : "+str(u["balance"])+"F\nMinimum : "+str(min_w)+"F\n\nChoisis ton pays :", kb=pays_keyboard())

    elif text == "\U00002705 Taches":
        if not ad_seen_today(uid):
            mark_ad_seen(uid)
            show_ad_then(uid, "ad_tasks", "Vois les taches et gagne des recompenses !")
            return
        tasks = db_get("tasks",{"date":f"eq.{today}","is_active":"eq.true"})
        ...
        if auto_done:         msg_text += "OK "+AUTO_TASK_DESC+"\nRecompense : "+str(AUTO_TASK_REW)+"F - VALIDEE\n\n"
        elif auto_pend_today: msg_text += "EN ATTENTE "+AUTO_TASK_DESC+"\nRecompense : "+str(AUTO_TASK_REW)+"F - En verification\n\n"
        else:                 msg_text += "-- "+AUTO_TASK_DESC+"\nLien : "+AUTO_TASK_LINK+"\nRecompense : "+str(AUTO_TASK_REW)+"F\n\n"
        msg_text += "================================\n\n"
        buttons = []
        if not auto_done and not auto_pend_today:
            buttons.append([{"text":"Valider : Sniper Business","callback_data":"auto_task_"+AUTO_TASK_ID}])
        for t in (tasks or []):
            ...
            if not done: buttons.append([{"text":"Valider : "+str(t["description"])[:30],"callback_data":"task_"+str(t["id"])}])
        if auto_done and not tasks: msg_text += "Toutes les taches completees ! Reviens demain."
        send(uid, msg_text, kb={"inline_keyboard":buttons} if buttons else None)

    elif text == "\U0001f3c6 Classement":
...
def handle_retrait_step(uid, text, sess):
    step = sess.get("step"); u = get_user(uid); min_w = get_cfg("min_withdrawal")
    pays = sess.get("pays",""); methode = sess.get("methode",""); crypto = is_crypto(pays, methode)
    if step == "amount":
        try: amount = int(text.strip().replace("f","").replace("F","").replace(" ",""))
        except: send(uid, "Montant invalide. Envoie un chiffre ex: 1000"); return
        if amount < min_w: send(uid, "Minimum : "+str(min_w)+"F"); return
        if amount > u["balance"]: send(uid, "Solde insuffisant. Ton solde : "+str(u["balance"])+"F"); return
        ok, msg = check_retrait_security(uid, amount, "")
        if not ok: send(uid, msg); clear_session(uid); return
        sess["amount"] = amount; sess["step"] = "number"; set_session(uid, sess)
        send(uid, "Montant : "+str(amount)+"F\n\n"+("Envoie ton adresse wallet :" if crypto else "Envoie ton numero de paiement :"))
    elif step == "number":
        min_len = 26 if crypto else 8
        if len(text.strip()) < min_len:
            send(uid, "Adresse wallet invalide (min 26 car.)" if crypto else "Numero invalide. Reessaie."); return
        number = text.strip()
        ok, msg = check_retrait_security(uid, sess.get("amount",0), number)
        if not ok: send(uid, msg); clear_session(uid); return
        check_multi_compte(uid, number)
        sess["number"] = number; sess["step"] = "name"; set_session(uid, sess)
        send(uid, "Enregistre !\n\nEnvoie ton nom complet :")
    elif step == "name":
        if len(text.strip()) < 3: send(uid, "Nom invalide. Reessaie."); return
        sess["name"] = text.strip()
        code = generate_code(); sess["step"] = "code"; sess["code"] = code; set_session(uid, sess)
        send(uid, "Code de confirmation :\n\nCODE : "+code+"\n\nSaisis ce code :")
    elif step == "code":
        if text.strip() != sess.get("code",""):
            send(uid, "Code incorrect. Reessaie.\n\nCode : "+sess.get("code","")); return
        amount = sess["amount"]; number = sess["number"]; name = sess["name"]
        masked = number if crypto else mask_number(number)
        update_balance(uid, -amount)
        r = db_post("withdrawals", {"user_id":uid,"amount":amount,"method":pays+" - "+methode,"number":number,"name":name,"status":"pending"})
        ...
        if RETRAIT_CHANNEL_ID and RETRAIT_CHANNEL_ID != "0":
            try:
                lbl = "Adresse wallet" if crypto else "Numero"
                tg("sendMessage", chat_id=int(RETRAIT_CHANNEL_ID),
...
def handle_admin_step(uid, text, sess):
    ...
    if action == "add_task":
        step = sess.get("step")
        if step == "description": sess["description"]=text; sess["step"]="link"; set_session(uid,sess); send(uid,"Lien (ou - si aucun) :")
        elif step == "link":
        ...
        if step == "user_id":
            try:
                target_id = int(text.strip()); u2 = get_user(target_id)
                if not u2: send(uid,"User introuvable."); clear_session(uid); return
                sess["target_id"]=target_id; sess["step"]="amount"; set_session(uid,sess)
                send(uid,"User : @"+str(u2.get("username","N/A"))+"\nSolde : "+str(u2["balance"])+"F\n\nCombien ajouter ?")
            except: send(uid,"ID invalide."); clear_session(uid)
        elif step == "amount":
            try:
                amount = int(text.strip().replace("f","").replace("F","").replace(" ",""))
                target_id = sess["target_id"]; update_balance(target_id, amount)
                ...
                try: send(target_id,"+"+str(amount)+"F credite !\nNouveau solde : "+str(new_u["balance"])+"F\n\nL equipe XAFEARN")
                except: pass
                clear_session(uid)
            except: send(uid,"Montant invalide."); clear_session(uid)
    elif action == "ban":
        t = text.strip()
        if t.startswith("debannir "):
            try:
                ...
                send(uid,"User "+str(tid)+" debanni."); send(tid,"Ton compte a ete reactive.")
            ...
            try:
                ...
                send(uid,"User "+str(tid)+" banni."); send(tid,"Ton compte a ete suspendu.")
            except: send(uid,"ID invalide.")
        clear_session(uid)
    elif action == "broadcast":
        users = db_get("users", {"is_registered":"eq.true"})
        sent  = 0
        kb = {"inline_keyboard": [[{"text": "Publier mon annonce", "web_app": {"url": MINIAPP_URL}}]]}
        for uu in users:
...
def handle_cb(uid, data, mid, cid):
    ...
    if data == "ad_bonus":
        ...
        bonus = get_cfg("bonus_daily")
        if str(u.get("last_bonus","")) == today:
            edit(uid, mid, "Bonus deja recupere !\nReviens demain pour +"+str(bonus)+"F"); return
        update_balance(uid, bonus)
        db_patch("users",{"user_id":f"eq.{uid}"},{"last_bonus":today})
        db_post("transactions",{"user_id":uid,"type":"bonus","amount":bonus,"description":"Bonus journalier"})
        new_u = get_user(uid)
        edit(uid, mid, "BONUS RECU !\n\n+"+str(bonus)+"F credite\nNouveau solde : "+str(new_u["balance"])+"F\n\nReviens demain !")
        notif_palier(uid, new_u["balance"])
        return
    ...
    if data.startswith("pays_"):
        idx = int(data.split("_")[1]); pays_list = list(PAYS_METHODES.keys())
        if idx >= len(pays_list): return
        pays = pays_list[idx]
        set_session(uid,{"action":"retrait","step":"methode","pays":pays})
        edit(uid,mid,"Pays : "+pays+"\n\nChoisis ta methode :",kb=methodes_keyboard(pays)); return

    if data.startswith("methode_"):
        parts = data.split("_",2); idx = int(parts[1]); pays = parts[2]
        methodes = PAYS_METHODES.get(pays,[])
        if idx >= len(methodes): return
        methode = methodes[idx]; u = get_user(uid); min_w = get_cfg("min_withdrawal")
        set_session(uid,{"action":"retrait","step":"amount","pays":pays,"methode":methode})
        edit(uid,mid,"Pays : "+pays+"\nMethode : "+methode+"\n\nSolde : "+str(u["balance"])+"F\nMinimum : "+str(min_w)+"F\n\nCombien veux-tu retirer ? (en F)"); return

    if data == "check_join":
        u = get_user(uid)
        if not u: return
        if not check_joined(uid):
            edit(uid, mid, "Tu n as pas encore tout rejoint.\nRejoins les 3 canaux puis clique Verifier.",
                kb={"inline_keyboard": [
                    [{"text": "Canal 1", "url": CHANNELS_DISPLAY[0]}],
                    [{"text": "Canal 2", "url": CHANNELS_DISPLAY[1]}],
                    [{"text": "Canal 3", "url": CHANNELS_DISPLAY[2]}],
        ...
        notif_bienvenue(uid, u.get("username",""))
        ...
        if done_today: tg("answerCallbackQuery",callback_query_id=str(mid),text="Tache deja completee !",show_alert=True); return
        ...
        if pend_today: tg("answerCallbackQuery",callback_query_id=str(mid),text="Demande deja envoyee ! Attends.",show_alert=True); return
        ...
        if not ws: edit(uid,mid,"Retrait introuvable."); return
        if ws[0].get("status") != "pending": edit(uid,mid,"Deja traite."); return
        w = ws[0]; method_full = w.get("method",""); masked = mask_number(w["number"])
        if decision == "approve":
            db_patch("withdrawals",{"id":f"eq.{w_id}"},{"status":"approved"})
            log_action(uid,"APPROVE_RETRAIT","w_id="+str(w_id)+" amount="+str(w["amount"]))
            edit(cid,mid,"PAIEMENT EFFECTUE\n\nMontant : "+str(w["amount"])+"F\nMethode : "+method_full+"\nNumero : "+w["number"]+"\nNom : "+str(w["name"])+"\n\nVia @"+BOT_USERNAME)
