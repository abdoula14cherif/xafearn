...
def main_kb():
    return {"keyboard": [
        ...
        ["Retrait", "Taches"],
        ["Classement", "Aide"],
        [{"text": "Ouvrir XAFEARN App", "web_app": {"url": MINIAPP_URL}}]
...
def admin_kb():
    return {"keyboard": [
        ...
        ["Supprimer Tache", "Pause Retraits"],
...
def check_retrait_security(uid, amount, number):
    ...
    if number:
        ...
        for blocked in BLACKLIST_NUMEROS:
            if clean == blocked.replace(" ",""):
                return False, "Une verification est en cours. Contacte le support : https://wa.me/699663183"
    ...
    if amount > 0:
        ...
        if today_total + amount > MAX_RETRAIT_PAR_JOUR:
            ...
            return False, "Montant depasse ta limite. Tu peux retirer au maximum " + str(remaining) + "F aujourd hui."
    return True, None
        ...
        for w in all_ws:
            if w.get("user_id") != uid:
                for admin_id in ADMIN_IDS:
                    try:
                        send(admin_id, "ALERTE MULTI-COMPTES\n\nNumero " + mask_number(number) +
...
def handle_msg(uid, uname, text):
    ...
    if uid in ADMIN_IDS:
        ...
        if text == "Broadcast":
            ...
            send(uid, "Ecris le message a envoyer a tous :\n\nLe bouton [Publier mon annonce] sera ajoute automatiquement."); return
