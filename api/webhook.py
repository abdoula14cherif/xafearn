...
def tg(method, **kw):
    try:
        return req.post(f"{API}/{method}", json=kw, timeout=10).json()
    except:
        return {}
...
def db_get(table, f={}):
    try:
        r = req.get(f"{DB}/{table}", headers=H, params=f, timeout=10)
        return r.json() or []
    except:
        return []
...
def db_post(table, data):
    try:
        r = req.post(f"{DB}/{table}", headers=H, json=data, timeout=10)
        return r.json()
    except:
        return []
...
def db_patch(table, f, data):
    try:
        req.patch(f"{DB}/{table}", headers=H, params=f, json=data, timeout=10)
    except:
        pass

def db_del(table, f):
    try:
        req.delete(f"{DB}/{table}", headers=H, params=f, timeout=10)
    except:
        pass
...
def get_cfg(key):
    ...
    return {"bonus_daily":100,"bonus_referral":75,"bonus_task":35,"min_withdrawal":500}.get(key,0)
...
def get_session(uid):
    ...
    if r:
        try: return json.loads(r[0]["data"])
        except: return {}
    return {}
...
def handle_msg(uid, uname, text):
    ...
    if uid in ADMIN_IDS:
        if text in ["/admin", "📊 Statistiques"]:
            ...
            send(uid,
                ...
                f"⏳ Retraits en attente : *{sum(1 for x in ws if x.get('status')=='pending')}*",
                kb=admin_kb()
            ); return
    ...
    if not u.get("is_registered"):
        send(uid, "⚠️ Rejoins nos canaux d'abord. Envoie /start"); return
    ...
    if text == "🎁 Bonus Journalier":
        ...
        if str(u.get("last_bonus")) == today:
            send(uid, f"⏳ *Bonus déjà récupéré !*\n\n💼 Solde : *{u['balance']}F*\n🔔 Reviens demain pour +{bonus}F"); return
        ...
        for t in tasks:
            ...
            if t.get("link"): msg_text += f"   🔗 _{t['link']}_\n"
...
def handle_cb(uid, data, mid, cid):
    if data == "check_join":
        ...
        if db_get("user_tasks",{"user_id":f"eq.{uid}","task_id":f"eq.{task_id}"}):
            tg("answerCallbackQuery",callback_query_id=str(mid),text="⚠️ Déjà complétée !",show_alert=True); return
